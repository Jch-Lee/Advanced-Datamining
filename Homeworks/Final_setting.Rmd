...
cv_model <- cv.glmnet(as.matrix(College[,-ncol(College)]),
                      College[,17], alpha = 1,
                      nfolds = 3)
pp <- predict(cv_model, newx = as.matrix(College[,-ncol(College)]), s="lambda.min")
RMSE(College$Grad.Rate, pp)
...
stack331 <- function(trainset, testset) {
  ...
  cv_pred_mat <- matrix(0, nrow=nrow(trainset), ncol=n_models1)
  test_pred_list <- lapply(1:n_models1, function(x) matrix(0,
                                                   nrow = nrow(testset),
                                                   ncol = max(stack_fold)))
  ...
  stage1_trainset <- data.frame(cv_pred_mat, y=trainset$y)
  stage1_testx <- data.frame(test_pred_mat)
  
  stage1_cv_pred_mat <- matrix(0, nrow=nrow(trainset), ncol=n_models2)
  stage1_test_pred_list <- lapply(1:n_models2,
                                  function(x) matrix(0, nrow = nrow(testset), ncol = max(stack_fold)))
  
  for (i in 1:max(stack_fold)) {
    fit1 <- cv.glmnet(apply(as.matrix(stage1_trainset[stack_fold!=i, -ncol(stage1_trainset)]),
                            2, scale),
                      stage1_trainset$y[stack_fold!=i],
                      alpha = 1,
                      nfolds = 3)
    fit2 <- gbm(y~.,
                data = stage1_trainset[stack_fold!=i,],
                distribution = "gaussian",
                n.trees = 250)
    fit3 <- randomForest(y~.,
                         data = stage1_trainset[stack_fold!=i,])
    ...
    stage1_cv_pred_mat[stack_fold==i,1] <-
      predict(fit1, newx = apply(as.matrix(stage1_trainset[stack_fold==i, -ncol(stage1_trainset)]),
                                 2, scale),
              s = "lambda.1se")
    stage1_cv_pred_mat[stack_fold==i,2] <-
      predict(fit2, newdata = stage1_trainset[stack_fold==i,])
    stage1_cv_pred_mat[stack_fold==i,3] <-
      predict(fit3, newdata = stage1_trainset[stack_fold==i,])
    
    stage1_test_pred_list[[1]][,i] <- 
      predict(fit1, newx = apply(as.matrix(stage1_testx),
                                 2, scale),
              s = "lambda.1se")
    stage1_test_pred_list[[2]][,i] <- 
      predict(fit2, newdata = stage1_testx)
    stage1_test_pred_list[[3]][,i] <- 
      predict(fit3, newdata = stage1_testx)
  }
...
RF_5fold <- function(data) {
  ...
  for (i in 1:max(fold)) {
    ...
    fit <- randomForest(y~., data = trainset)
