...
![Summary](C:/Users/jongcheol/OneDrive/바탕 화면/Semester_2/Advanced-Datamining/Homeworks/결과테이블.png)

##### 결과 해석
* 본 결과의 최소 **CVE** 관점에서는 예측함수 **g**와 거리함수 **d3**의 조합이 가장 좋았다. 
* 하지만 예측함수 **h**는 *test sample*과 특징이 더 가까울 수록 큰 가중치를 부여하여, 주변 샘플들의  *target(crim)* 값 가중평균을 예측값으로 사용하므로, 그래프를 비교하였을 때 **K**가 증가해도 가장 안정적인 예측 성능을 보였다. 
