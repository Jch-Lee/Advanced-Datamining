...
lines(res3[[2]][,1])
legend("topright", legend=c("dist3", "dist2", "dist1"), 
...
![My Image](C:/Users/USER/Desktop/Semester2/Advanced-Datamining/Homeworks/결과테이블.png)

##### 결과 해석
* 본 결과의 최소 **CVE** 관점에서는 예측함수 **g**와 거리함수 **d3**의 조합이 가장 좋았다. 
* 하지만 예측함수 **h**는 *test sample*과 더 가까울 수록 큰 가중치를 부여하여 주변 *target(crim)* 값들의 *test sample*의 가중평균을 예측값으로 사용하므로, 그래프를 비교하였을 때 **K**가 증가해도 가장 안정적인 예측 성능을 보였다. 또한 **h**는 거리함수 **d3**를 사용했을 때, 전반적으로 낮은 **CVE**값을 나타냈다.
* 따라서 예측의 안정성까지 고려했을 때는 예측함수 **h**와 거리함수 **d3**를 조합한 방법을 사용하는 것이 가장 좋다.
