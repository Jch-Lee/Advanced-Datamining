...
load("C:/Users/jongcheol/OneDrive/바탕 화면/Semester2/Advanced-Datamining/Homeworks/exam.Rdata")
```
...
```{r}
boxplot(trainset$y[trainset$X10==1],
        trainset$y[trainset$X10==2],
        trainset$y[trainset$X10==3],
        trainset$y[trainset$X10==4])
```
...
SVM_5fold <- function(data) {
  ...
  for (i in 1:max(fold)) {
    ...
    trainset <- apply(trainset, 2, as.numeric)
    validset <- apply(validset, 2, as.numeric)
    
    # SVM 회귀 모델 생성
    svm_model <- svm(trainset[,-ncol(trainset)],
                     trainset[,ncol(trainset)],
                     type = "eps-regression", kernel = "radial")
    # 모델 예측
    y_pred <- predict(svm_model, validset[,-ncol(validset)])

    performance[i] <- RMSE(validset[,ncol(validset)], y_pred)
  }
  plot(performance, type="l")
...
```{r}
NN_5fold <- function(data, size=5, maxit=500) {
  library(nnet)
  
  # 5-폴드 인덱스 생성
  fold <- sample(rep(1:5, length.out=nrow(data)))
  
  performance <- numeric(5)
  for (i in 1:5) {
    trainset <- data[fold != i, -c(6,8,9,10,12)]
    validset <- data[fold == i, -c(6,8,9,10,12)]
    
    # nnet 사용, 회귀 문제이므로 linout=TRUE
    # size = 은닉층 뉴런 수, maxit=최대 반복 횟수
    fit <- nnet(y ~ ., data=trainset, size=30, linout=TRUE,
